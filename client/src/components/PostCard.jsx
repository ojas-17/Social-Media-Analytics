...
function PostCard({ post }) {
    ...
    return (
        <div className='w-full sm:w-1/2 md:w-1/3 xl:w-1/4 p-2'>
            <div className={`w-full flex flex-col rounded-xl border-4 transition-transform duration-200 hover:scale-105 ${theme === 'light' ? 'border-green-400 bg-neutral-300' : 'border-green-600 bg-neutral-900'}`}>
                <Link className={`w-full`} to={`/posts/${post.id}`}>
                    <img className='object-cover rounded-t-lg aspect-[4/3]' src={post?.media_url} alt="" />
                </Link>
                <Link to={`/posts/${post.id}`} className='w-full h-20 p-2 text-lg'>
                    {post?.caption}
                </Link>
